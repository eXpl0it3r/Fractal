...
Application::Application() :
    m_window{{1000u, 765u}, "Fractal - Mandelbrot"},
    m_fractal{{1000u, 765u}, 4u},
    m_down{-1, -1},
    m_resized{true},
    m_sidebarWidth{0.f}
{
    ...
    {
        sf::err() << "Failed to load font" << std::endl;
    }

    m_wait.setFont(m_font);
    m_wait.setCharacterSize(30u);
    m_wait.setString("Please Wait!");
    m_wait.setPosition(static_cast<sf::Vector2f>(m_window.getSize()) / 2.f - m_wait.getLocalBounds().getSize() / 2.f);
    ...
    // Init fractal
    regenerate({0, 0}, sf::Vector2i{
                   static_cast<int>(m_window.getSize().x + m_sidebarWidth), static_cast<int>(m_window.getSize().y)
               }, false);
}
...
void Application::createGUI()
{
    ...
    m_desktop.GetEngine().GetResourceManager().AddFont("DejaVuSans", std::make_shared<sf::Font>(m_font));
    ...
    m_divisorLabel = sfg::Label::Create("Divisor");
    m_divisorEntry = sfg::Entry::Create();
    m_columnRightBox->Pack(m_divisorLabel);
    m_columnRightBox->Pack(m_divisorEntry);
    ...
    m_verticalBox->SetRequisition({160.f, static_cast<float>(m_window.getSize().y)});
    ...
    m_sidebar->SetAllocation({
        static_cast<float>(m_window.getSize().x) - m_sidebarWidth, 0.f, 300.f, static_cast<float>(m_window.getSize().y)
    });
...
void Application::update()
{
    for (auto event = sf::Event{}; m_window.pollEvent(event);)
    {
        ...
        else if (event.type == sf::Event::MouseButtonReleased && event.mouseButton.button == sf::Mouse::Left)
        {
            ...
            {
                // Update fractal
                regenerate(m_down, {event.mouseButton.x, event.mouseButton.y});
        ...
        else if (event.type == sf::Event::KeyPressed)
        {
            if (event.key.code == sf::Keyboard::Up || event.key.code == sf::Keyboard::Down)
            {
                ...
                m_precisionEntry->SetText(std::to_string(static_cast<int>(m_fractal.precision())));
            }
        }
    ...
    if (m_down.x != -1 && m_down.y != -1)
    {
        ...
        m_select.setSize(sf::Vector2f{sf::Mouse::getPosition(m_window) - m_down});
    }
    else
    {
        m_select.setSize({0.f, 0.f});
    }
}
...
void Application::regenerate(const sf::Vector2i& first, const sf::Vector2i& second, const bool fractal)
{
    // Set entry values
    m_zoomEntry->SetText(std::to_string(static_cast<double>(m_fractal.zoom())));
    m_xEntry->SetText(std::to_string(static_cast<double>(m_fractal.position().x)));
    m_yEntry->SetText(std::to_string(static_cast<double>(m_fractal.position().y)));
    m_precisionEntry->SetText(std::to_string(static_cast<int>(m_fractal.precision())));
    m_divisorEntry->SetText(std::to_string(static_cast<int>(m_fractal.divisor())));
...
void Application::onResize()
{
    ...
    // Resize widgets
    m_sidebar->SetAllocation({size.x - m_sidebarWidth, 0, 300, size.y});
    ...
    regenerate({0, 0}, sf::Vector2i{static_cast<int>(size.x + m_sidebarWidth), static_cast<int>(size.y)}, false);
}
